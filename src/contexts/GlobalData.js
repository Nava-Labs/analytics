...
function reducer (state, { type, payload }) {
  switch (type) {
    ...
    case UPDATE_ETH_PRICE: {
      const { mntPrice, oneDayPrice, ethPriceChange } = payload
      return {
        [ETH_PRICE_KEY]: mntPrice,
        oneDayPrice,
        ethPriceChange
      }
    }
...
export default function Provider ({ children }) {
  ...
  const updateEthPrice = useCallback((mntPrice, oneDayPrice, ethPriceChange) => {
    dispatch({
...
async function getGlobalData (mntPrice, oldEthPrice) {
  ...
  try {
    ...
    if (data && oneDayData && twoDayData && twoWeekData) {
      ...
      const liquidityChangeUSD = getPercentChange(
        ...
        oneDayData.totalLiquidityMNT * oldEthPrice
      )
...
const getEthPrice = async () => {
  ...
  return [mntPrice, ethPriceOneDay, priceChangeMNT]
}
...
/**
 * Loop through every pair on Esper, used for search
 */
...
/**
 * Loop through every token on Esper, used for search
 */
...
export function useGlobalData () {
  ...
  const [mntPrice, oldEthPrice] = useEthPrice()
  ...
  useEffect(() => {
    async function fetchData () {
      let globalData = await getGlobalData(mntPrice, oldEthPrice)
    ...
    if (!data && mntPrice && oldEthPrice) {
      fetchData()
    }
  }, [mntPrice, oldEthPrice, update, data, updateAllPairsInUniswap, updateAllTokensInUniswap])
  return data || {}
}
...
export function useEthPrice () {
  ...
  const mntPrice = state?.[ETH_PRICE_KEY]
  ...
  useEffect(() => {
    ...
    checkForEthPrice()
  }, [mntPrice, updateEthPrice])
  return [mntPrice, ethPriceOld]
}
