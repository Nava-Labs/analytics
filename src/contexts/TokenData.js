...
const getTopTokens = async (mntPrice, ethPriceOld) => {
    ...
    let bulkResults = await Promise.all(
      current &&
        ...
        current?.data?.tokens.map(async token => {
          ...
          const oldLiquidityUSD = oneDayHistory?.totalLiquidity * ethPriceOld * oneDayHistory?.derivedMNT

          // percent changes
          const priceChangeUSD = getPercentChange(
            data?.derivedMNT * mntPrice,
            oneDayHistory?.derivedMNT ? oneDayHistory?.derivedMNT * ethPriceOld : 0
          )
...
const getTokenData = async (address, mntPrice, ethPriceOld) => {
  const utcCurrentTime = dayjs()
  ...
  try {
    ...
    const priceChangeUSD = getPercentChange(
      data?.derivedMNT * mntPrice,
      parseFloat(oneDayData?.derivedMNT ?? 0) * ethPriceOld
    )

    const currentLiquidityUSD = data?.totalLiquidity * mntPrice * data?.derivedMNT
    const oldLiquidityUSD = oneDayData?.totalLiquidity * ethPriceOld * oneDayData?.derivedMNT
...
export function Updater () {
  ...
  const [mntPrice, ethPriceOld] = useEthPrice()
  useEffect(() => {
    async function getData () {
      // get top pairs for overview list
      let topTokens = await getTopTokens(mntPrice, ethPriceOld)
      topTokens && updateTopTokens(topTokens)
    }
    mntPrice && ethPriceOld && getData()
  }, [mntPrice, ethPriceOld, updateTopTokens])
  return null
}
...
export function useTokenData (tokenAddress) {
  ...
  useEffect(() => {
    if (!tokenData && mntPrice && ethPriceOld && isAddress(tokenAddress)) {
      getTokenData(tokenAddress, mntPrice, ethPriceOld).then(data => {
        update(tokenAddress, data)
      })
    }
  }, [mntPrice, ethPriceOld, tokenAddress, tokenData, update])
...
export function useTokenDataCombined (tokenAddresses) {
  ...
  useEffect(() => {
    async function fetchDatas () {
      Promise.all(
        tokenAddresses.map(async address => {
          return await getTokenData(address, mntPrice, ethPriceOld)
        })
      )
        .then(res => {
          if (res) {
    ...
    if (!volume && mntPrice && ethPriceOld) {
      fetchDatas()
    }
  }, [tokenAddresses, mntPrice, ethPriceOld, volume, updateCombinedVolume])
